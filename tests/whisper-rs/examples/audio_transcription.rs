...
fn main() -> Result<(), &'static str> {
    ...
    // Note: Custom aheads are commented out because they were configured for base model (5 layers)
    // but we're using tiny model (4 layers). The model preset approach is more reliable.
    // 
    ...
    let ctx = WhisperContext::new_with_params(
        "D:\\Irak\\VoipGlot\\voipglot-win\\tests\\whisper-rs\\ggml-tiny.bin",
        context_param,
    )
    ...
    // Open the audio file.
    let reader = hound::WavReader::open("samples_jfk.wav").expect("failed to open file");
